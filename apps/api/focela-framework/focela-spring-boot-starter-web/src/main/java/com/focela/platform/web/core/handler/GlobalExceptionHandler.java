...
import static com.focela.platform.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[Report module focela-report - table schema not imported][see https://platform.focela.com/report/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[Report module focela-report - table schema not imported][see https://platform.focela.com/report/ to enable]");
        }
        // 2. Workflow module
        if (message.contains("bpm_")) {
            log.error("[Workflow module focela-bpm - table schema not imported][see https://platform.focela.com/bpm/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[Workflow module focela-bpm - table schema not imported][see https://platform.focela.com/bpm/ to enable]");
        }
        // 3. WeChat MP
        if (message.contains("mp_")) {
            log.error("[WeChat MP focela-mp - table schema not imported][see https://platform.focela.com/mp/build/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[WeChat MP focela-mp - table schema not imported][see https://platform.focela.com/mp/build/ to enable]");
        }
        // 4. Mall system
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[Mall system focela-mall - is disabled][see https://platform.focela.com/mall/build/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[Mall system focela-mall - is disabled][see https://platform.focela.com/mall/build/ to enable]");
        }
        // 5. ERP system
        if (message.contains("erp_")) {
            log.error("[ERP system focela-erp - table schema not imported][see https://platform.focela.com/erp/build/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[ERP system focela-erp - table schema not imported][see https://platform.focela.com/erp/build/ to enable]");
        }
        // 6. CRM system
        if (message.contains("crm_")) {
            log.error("[CRM system focela-crm - table schema not imported][see https://platform.focela.com/crm/build/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[CRM system focela-crm - table schema not imported][see https://platform.focela.com/crm/build/ to enable]");
        }
        // 7. Payment platform
        if (message.contains("pay_")) {
            log.error("[Payment module focela-pay - table schema not imported][see https://platform.focela.com/pay/build/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[Payment module focela-pay - table schema not imported][see https://platform.focela.com/pay/build/ to enable]");
        }
        // 8. AI large model
        if (message.contains("ai_")) {
            log.error("[AI large model focela-ai - table schema not imported][see https://platform.focela.com/ai/build/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[AI large model focela-ai - table schema not imported][see https://platform.focela.com/ai/build/ to enable]");
        }
        // 9. IoT
        if (message.contains("iot_")) {
            log.error("[IoT focela-iot - table schema not imported][see https://platform.focela.com/iot/build/ to enable]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IoT focela-iot - table schema not imported][see https://platform.focela.com/iot/build/ to enable]");
        }
        return null;
    }
