package com.focela.platform.framework.test.core.utils;
...
public class RandomUtils {
    ...
    static {
        ...
        PODAM_FACTORY.getStrategy().addOrReplaceTypeManufacturer(Integer.class, (dataProviderStrategy, attributeMetadata, map) -> {
            // for a status field, return 0 or 1
            if ("status".equals(attributeMetadata.getAttributeName())) {
                return RandomUtil.randomEle(CommonStatusEnum.values()).getStatus();
            }
            // for fields ending in type or status, return within the tinyint range
        ...
        // BigDecimal: limit precision within DECIMAL(10,2) to avoid overflow on databases like H2
        ...
        PODAM_FACTORY.getStrategy().addOrReplaceTypeManufacturer(Boolean.class, (dataProviderStrategy, attributeMetadata, map) -> {
            // for a deleted field, return not-deleted
    ...
    public static LocalDateTime randomLocalDateTime() {
        // set Nano to zero to avoid MySQL/H2 losing the timestamp
    ...
    public static String randomURL() {
        return "https://www.example.com/" + randomString();
    }

    @SafeVarargs
    public static <T> T randomPojo(Class<T> clazz, Consumer<T>... consumers) {
        T pojo = PODAM_FACTORY.manufacturePojo(clazz);
        // when non-empty, run the callback logic; this allows further processing of the Pojo
